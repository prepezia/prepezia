...
function AddSourcesDialog({ open, onOpenChange, onAddSources }: { open: boolean, onOpenChange: (open: boolean) => void, onAddSources: (sources: Source[]) => void }) {
    ...
    const handleAddCopiedText = () => {
        ...
        setSources(prev => [...prev, { type: 'clipboard', name: `Pasted Text (${copiedText.substring(0, 15)}...)`, data: copiedText, contentType: 'text/plain' }]);
        setCopiedText("");
    ...
    const handleAddUrl = () => {
        ...
        setSources(prev => [...prev, { type: urlModalConfig.type, name: currentUrl, url: currentUrl }]);
        setCurrentUrl("");
    ...
    const handleAddSelectedSources = () => {
        const newSources: Source[] = selectedWebSources.map(result => ({
            type: (result.url.includes('youtube.com') || result.url.includes('youtu.be')) ? 'youtube' : 'website',
            name: result.title,
            url: result.url
        }));
        if (newSources.length > 0) {
            onAddSources(sources.concat(newSources));
        }
    ...
    return (
        <>
            <Dialog open={open} onOpenChange={(val) => { if (!isSearchModalOpen && !isUrlModalOpen && !isTextModalOpen) onOpenChange(val) }}>
                <DialogContent className="sm:max-w-3xl">
                    <DialogHeader>
                        <DialogTitle>Add More Sources</DialogTitle>
                        <DialogDescription>Add new files, links, or text to your study space.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-8 py-4">
                        <div className="space-y-6">
                            <div className="flex w-full items-center space-x-2">
                                <Input placeholder="Search the web for articles, videos..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleSearch(); } }} />
                                <Button onClick={handleSearch} disabled={isSearching} size="icon"><Search className="h-4 w-4" /></Button>
                            </div>
                            <div className="relative py-2">
                                <div className="absolute inset-0 flex items-center" aria-hidden="true"><div className="w-full border-t" /></div>
                                <div className="relative flex justify-center"><span className="bg-card px-2 text-sm text-muted-foreground">Or Add Manually</span></div>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {sourceButtons.map(btn => (<Button key={btn.name} variant="outline" className="h-20 text-base flex-col" onClick={btn.action}><btn.icon className="mb-1 h-6 w-6"/>{btn.name}</Button>))}
                            </div>
                            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" />
                        </div>
                        {sources.length > 0 && (
                            <div className="space-y-2">
                                <h4 className="font-semibold">Staged Sources ({sources.length})</h4>
                    ...
                    <DialogFooter>
                        <Button onClick={handleFinish} size="lg" disabled={sources.length === 0}>Add {sources.length > 0 ? `${sources.length} Source(s)` : 'Sources'}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
                                                ...
                                                return (
                                                    <div key={option.type} className="flex items-center justify-between p-2 rounded-md bg-secondary/50 hover:bg-secondary">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <Button size="icon" variant="outline" onClick={() => setActiveGeneratedView(option.type as any)}>
